...
$BlogViewer = new Com_Blog_Viewer($DBConfig);
            ...
            foreach ($UModule->LoadModule($_SESSION["UserID"], Com_User_LoadModule::Layout_Head) as $value) {
                ...
                <script>
                    ...
                    ss.DocumentReady(function () {
                        ...
                        ss.S("#BNReEnterPass").Click(function (e) {
                            var id = this.getAttribute("data-id");
                            sd.Login(function (pw) {
                                ss.Post("../../../Api/Ajax/Blog/AuthBlogID.php", {"ID": id, "UserName": pw["UserName"], "Password": pw["Password"]}, function (data) {
                                    if (data == "1") {
                                        window.location = "BlogViewer.php?id=" + id;
                                    } else {
                        ...
                        ss.S("#SearchRS").Click(function (e) {
                            if (e.target.getAttribute("data-password") == "1") {
                                sd.Login(function (pw) {
                                    ss.Post("../../../Api/ShareAjax/Blog/AuthBlogID.php", {"ID": e.target.getAttribute("data-id"), "UserName": pw["UserName"], "Password": pw["Password"]}, function (data) {
                                        if (data == "1") {
                                            window.location = "BlogViewer.php?id=" + e.target.getAttribute("data-id");
                                        } else {
                                            sd.Alert("Password Incorrect").ZIndex(1000);
                                        }
                                    });
                        ...
                        wsl.Done=(function (data) {
                            ...
                            for (var i = 0; i < data.length; i++) {
                                ...
                                if (data[i]["haspassword"] === undefined || data[i]["haspassword"] == "0") {
                                    q.Append("<h3></h3>").Append("<a class='LinkOpen'></a>").Url("BlogViewer.php?id=" + data[i]["id"]).Append(data[i]["title"]);
                                } else {
                ...
                <div class="Container">
                    ...
                    <div class="Section">
                        ...
                        <div id="SearchRS">
                            ...
                            if (isset($_GET["Category"])) {
                                ...
                                $Dat = array_merge($BlogViewer->GetBlogListByCategoryIDNotUserID($_SESSION["UserID"], Config_DB_Config::Access_Mode_Public), $BlogViewer->GetBlogListByCategoryIDNotUserID($_SESSION["UserID"], Config_DB_Config::Access_Mode_Members));
                                foreach ($Dat as $value) {
                                ...
                                if (intval($value["haspassword"]) == "1") {
                                    ...
                                    printf('<div class="BlogList"><h3><a class="LinkOpen" href="BlogViewer.php?id=%s">%s</a></h3>%s</div>', $value["id"], $value["title"], $value["description"]);
                                    }
                                }
                            } else if (!isset($_GET["id"])) {

                            $Dat = array_merge($BlogViewer->GetSimpleLastBlogListNotUserID($_SESSION["UserID"], Config_DB_Config::Access_Mode_Public), $BlogViewer->GetSimpleLastBlogListNotUserID($_SESSION["UserID"], Config_DB_Config::Access_Mode_Members));
                            foreach ($Dat as $value) {
                                if (isset($_SESSION["Blog_Password"][$value["id"]])) {
                                    $value["haspassword"] = 0;
                                }
                    ...
                    <div class="Aside">
                        <div class="BorderBlock" style="margin-top: 1px;">
                            ...
                            <a href="BlogViewer.php">View Last Blog</a>
                        </div>
                        <div class="BorderBlock" style="margin-top: 1px;">
                            ...
                            <ul>
                                ...
                                foreach ($Category->GetAllCategory() as $value) {
                                    printf('<li><a href="BlogViewer.php?Category=%s">%s</a></li>', $value["id"], $value["name"]);
                                }
                                ?>
                            </ul>
                        </div>
                        ...
                        if (isset($_GET["id"])) {
                            ...
                            echo '<label class="Title">Reader</label>';
                        printf('<a id="BNReEnterPass" href="#" data-id="%s">Re Enter Password</a>', $_GET["id"]);
                        echo '</div>';
                    }
