        ...
        <head>
            ...
            <script>
                ...
                ss.DocumentReady(function () {
                    ...
                    fl.currentdir = "/";
                    fl.DownloadURL = "../../../Api/Action/Files/TrashDownloadFile.php?id=";

                    fl.ChDir = function (v) {
                        ss.Post("../../../Api/Ajax/FilesTrash/GetFiles.php", {"Location": v}, function (data) {
                            fl.currentdir = v;
                            fl.ClearFileList();
                            ...
                            for (var i in data) {
                                ...
                                if (["jpg", "gif", "png", "jpeg"].indexOf(ext) >= 0) {
                                    fl.AddFile(data[i]["name"], data[i]["fullpath"], "../../../Api/Action/Files/TrashImagePreview.php?id=" + data[i]["fullpath"], data[i]["size"], data[i]["modified"], data[i]["type"]);
                                } else {
                    ...
                    fl.OpenFile = function (v) {
                        ...
                        if (["mp4", "webm", "ogg"].indexOf(ext.toLowerCase()) >= 0) {
                            var player = dialog.VideoPlayer("../../../Api/Action/Files/TrashDownloadFile.php?id=" + btoa(v))
                            player.ZIndex(999);
                            player.Width("800px");
                            player.Height("600px");
                        } else if (["mp3", "wma"].indexOf(ext.toLowerCase()) >= 0) {
                            var player = dialog.AudioPlayer("../../../Api/Action/Files/TrashDownloadFile.php?id=" + btoa(v));
                            player.ZIndex(999);
                            player.Width("800px");

                        } else if (["jpg", "gif", "png", "jpeg"].indexOf(ext.toLowerCase()) >= 0) {
                            dialog.ImageViewer("../../../Api/Action/Files/TrashDownloadFile.php?id=" + btoa(v)).ZIndex(999);
                        }
                    };
                    fl.PropertiesFile = function (v) {
                        ss.Post("../../../Api/Ajax/FilesTrash/GetPropertiesFile.php", {"Path": v}, function (data) {
                            data = JSON.parse(data);
                    ...
                    ss.S("#BNDelete").Click(function () {
                        ...
                        if (s.length>0) {
                            dialog.Confirm("Delete It????", function (name) {
                                ss.Post("../../../Api/Ajax/FilesTrash/DeleteFile.php", {"Files": (s)}, function (data) {
                                    fl.ChDir(fl.currentdir);
                                });
                            }).ZIndex(999);
                        }
                    ...
                    ss.S("#BNEmpty").Click(function () {
                        dialog.Confirm("Empty ????", function (name) {
                            ss.Post("../../../Api/Ajax/FilesTrash/DeleteFile.php", {"Files": (["/"])}, function (data) {
                                fl.ChDir(fl.currentdir);
                            });
                        }).ZIndex(999);
                    });
                    ...
                    ss.S("#BNRestore").Click(function () {
                        var s = fl.GetSelectFiles();
                        ss.Post("../../../Api/Ajax/FilesTrash/Restore.php", {"Files": (s)}, function (data) {
                            fl.ChDir(fl.currentdir);
                        });
                    });
                    ss.S("#BNRestoreAll").Click(function () {
                        ss.Post("../../../Api/Ajax/FilesTrash/Restore.php", {"Files": (["/"])}, function (data) {
                            fl.ChDir(fl.currentdir);
                        });
                    });
                    ss.S("#TXTSearch").Input(function () {
                        ss.Post("../../../Api/Ajax/FilesTrash/SearchFileName.php", {"Location": fl.currentdir, "Name": this.value}, function (data) {
                            fl.ClearFileList();
                            data = JSON.parse(data);
                            for (var i in data) {
                                var ext = data[i]["ext"];
                ...
                <div class="Nav" >
                    ...
                    <div class="BorderBlock" style="margin-top: 1px;">
                        ...
                        <ul>
                            ...
                            <li style="font-weight: bold;">  Trash</li>
                        </ul>
                    </div>
                    <div class="BorderBlock" style="margin-top: 1px;">
                ...
                <div class="Section" >
                    ...
                    <div style="display: flex;flex-direction: row;margin-top: 7px;">
                        <div style="width: 100%; box-sizing: border-box;">
                            <input id="TXTSearch" placeholder="Search" style="width: 100%;box-sizing: border-box;" type="text"  />
                        </div>
                    </div>
                    ...
                    <div class="BorderBlock" >
                        <ul>
                            ...
                            if ($Permission->Writable($_SESSION["UserID"])) {
                                ...
                                <li> <a id="BNRestore" href="#">Restore</a></li>
                                <li> <a id="BNRestoreAll" href="#">Restore All</a></li>
