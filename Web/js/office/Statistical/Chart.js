class Chart {
    ...
    PaintXYScale(data) {
        ...
        var w80 = Math.floor(ctx.canvas.width * 0.8);
        ...
        for (var i = 1; i < maxycount; i++) {
            ...
            ctx.lineTo(ctx.canvas.width, ystring);
            ystring -= ystep;
        }
        ...
        ctx.lineTo(ctx.canvas.width, ystring);
        ctx.stroke();
        var labelwidth = (ctx.canvas.width - offsetleft) / (Object.keys(data).length + 1);
    ...
    DrawPieChart() {
        ...
        var fontsize = parseInt(ctx.font.match(/\d+/g).map(Number));
        var maxdata = Math.floor((ctx.canvas.height - fontsize - fontsize) / fontsize) - 1;
        ...
        var angle = 0;
        var radius = Math.min(w80 / 2, ctx.canvas.height / 2);
        var txty = fontsize;
        ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);
        for (var k in data) {
            var rcolor = 'rgb(' +
